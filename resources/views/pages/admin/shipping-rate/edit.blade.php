...
<div class="container justify-center">
    ...
    <div class="form-card">
        ...
        <p class="text-muted mb-4">Modify existing shipping charges and locations.</p>

        @include('components.alert')

        <form class="adduser-form" method="POST" action="{{ route('admin.shipping-rates.update', $shippingRate->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select
                        ...
                        @foreach(array_keys($countriesData) as $countryName)
                            <option value="{{ $countryName }}" {{ old('country', $shippingRate->country) === $countryName ? 'selected' : '' }}>
                                {{ $countryName }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="status" class="input-form @error('status') is-invalid @enderror" required>
                        <option value="1" {{ old('status', $shippingRate->status ? '1' : '0') == '1' ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ old('status', $shippingRate->status ? '1' : '0') == '0' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <div class="error-text text-danger mt-1">{{ $message }}</div>
                    @enderror
            ...
            <div class="d-flex align-items-center gap-3 mt-4">
                <button class="btn-submit" type="submit">
                    <i class="fas fa-save me-1"></i> Update Shipping Rate
                </button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const currentCountry = @json(old('country', $shippingRate->country));
    const currentCity = @json(old('city', $shippingRate->city));
