...
<div class="show-card">
  <div class="content-area">
    <p style="color: gray; font-style: italic; text-align: center; margin-bottom: 12px;">
      This transaction is associated with {{ $transaction->branch->name ?? 'Main' }} branch
    </p>
    ...
    <div class="info-list">
      <div class="info-group">
        <span class="i-label">Due Before Transaction</span>
        <span class="i-value" style="color: var(--text-main); font-weight:700;">
          {{ $transaction->due_before_transaction !== null ? number_format($transaction->due_before_transaction, 2) . ' TK' : 'N/A' }}
        </span>
      </div>

      <div class="info-group">
        <span class="i-label">Due After Transaction</span>
        <span class="i-value" style="color: var(--danger-color); font-weight:700;">
          {{ $transaction->due_after_transaction !== null ? number_format($transaction->due_after_transaction, 2) . ' TK' : 'N/A' }}
        </span>
      </div>
    </div>
    ...
    <div class="statement">
      <p class="statement-text">
        @if($isPayment)
          ...
          A payment of <strong>{{ number_format($transaction->amount, 2) }} TK</strong> was disbursed to <strong>{{ $transaction->supplier->company_name ?? 'Supplier' }}</strong>. The payable due balance was decreased accordingly.
        @elseif($isReturn)
          <strong style="color:#b91c1c;">Stock Return Adjustment:</strong>
          Stock return worth <strong>{{ number_format($transaction->amount, 2) }} TK</strong> was deducted from <strong>{{ $transaction->supplier->company_name ?? 'Supplier' }}</strong> payable balance.
        @elseif($isPurchase)
          <strong style="color:#7c3aed;">Stock Purchase Entry:</strong>
          Stock in worth <strong>{{ number_format($transaction->amount, 2) }} TK</strong> was purchased and added to <strong>{{ $transaction->supplier->company_name ?? 'Supplier' }}</strong> outstanding balance.
        @else
  ...
  <div class="card-footer-actions d-flex justify-content-end gap-2 p-3">
    ...
    @if($isReturn && $stockCutId)
      <a href="{{ route('admin.stock.cut.cut.show', $stockCutId) }}" class="btn-smart btn-red">
        <i class="fa-solid fa-arrow-rotate-left"></i> View Stock Cut (BRSKR{{ $stockCutId }})
      </a>
    @endif
  </div>
...
<div style="text-align:center; margin-top:20px;">
  <a href="{{ route('admin.supplier-transactions.index') }}" style="color:var(--text-muted); text-decoration:none;">
