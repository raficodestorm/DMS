    ...
    <form class="adduser-form" method="POST" action="{{ route('sr.payments.update', $payment->id) }}">
      ...
      <div class="col-md-12 mb-3">
        ...
        <input type="hidden" name="customer_id" id="selected_customer_id"
               value="{{ old('customer_id', $payment->customer_id) }}" required>
        ...
        <div class="cls-container" id="cls-container">
            <div class="cls-input-wrap">
                ...
                <input
                    ...
                    id="cls-input"
                    class="cls-input"
                    placeholder="Type shop name (or 2 spaces to show all customers with due)..."
                    autocomplete="off"
                    inputmode="search">
            </div>
      ...
      <div
        ...
        <small style="color: #744210;">
          ...
          Changing the amount will require a new review by the manager.
        </small>
      </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Auto pre-select current payment's customer on page load
    var preselectedId = parseInt(selectedCustomerId.value);
    if (preselectedId) {
        // Look up in allCustomers first (due > 0 list)
        var found = allCustomers.find(function(c) { return c.id === preselectedId; });
        if (found) {
            selectCustomer(found);
        } else {
            // Customer might have due = 0 now (edge case), show with note
            clsSelectedShop.textContent = 'Current Customer (ID: ' + preselectedId + ')';
            clsSelectedDue.textContent = 'Due data unavailable';
            clsContainer.style.display = 'none';
            clsSelectedBox.style.display = 'flex';
        }
    }
    ...
    if (amountInput) {
        ...
        // Trigger on page load if amount is pre-filled (old value or existing)
