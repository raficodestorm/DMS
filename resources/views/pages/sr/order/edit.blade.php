...
@push('scripts')
<script>
  // index starts after existing server-rendered items to avoid name conflicts
  ...
  function addProductCard(productId, name, stock, imageName) {
    ...
    let userRole = "{{ auth()->user()->role ?? 'sr' }}";
    $.get(`/${userRole}/get-product-data/${productId}`, function(data) {
